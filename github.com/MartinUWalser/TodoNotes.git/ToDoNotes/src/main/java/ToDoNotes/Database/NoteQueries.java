...
public class NoteQueries {
	...
	public static ArrayList<Note> selectVisibleNotes(ArrayList<Note> list, boolean visible) {
		...
		if (visible) {
            resultList.addAll(list.stream().filter(Note::isVisible).collect(Collectors.toList()));
		} else {
            resultList.addAll(list.stream().filter(e -> !e.isVisible()).collect(Collectors.toList()));
		}
		return resultList;
	}
	...
	public static ArrayList<Note> selectDoneNotes(ArrayList<Note> list, boolean done) {
		...
		if (done) {
            resultList.addAll(list.stream().filter(Note::isDone).collect(Collectors.toList()));
		} else {
            resultList.addAll(list.stream().filter(e -> !e.isDone()).collect(Collectors.toList()));
		}
		return resultList;
	}
	...
	public static void removeNote(Note note) {
		long id = note.getId();
		GroupQueries.deleteRelation(id);
		Connection conn = MySQLDAO.getConnection();
		PreparedStatement pS;
	...
	public static void setGroup(Note note) {
		...
		try {
			...
			while (groupRS.next())
				note.setGroup(GroupQueries.getGroup(groupRS.getLong("group_id")));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			try {
